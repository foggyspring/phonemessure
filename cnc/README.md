...
| 参考数据 | `cnc/engine/shopdata.py` + `cnc/data/*.json` | 材料密度/单价、机床时租、CAPP 常数、利润率（生产环境可换 PostgreSQL） |
...
| `POST /api/quote` | `multipart`：`file`（选填）+ `params`(JSON) → 完整报价 payload |
...
无文件时用 `manual_dims` 兜底（STEP 无内核时前端自动切到此路径）。
...
- **第三阶段**：材料单价对接供应商 API / 后台维护面板（现为 JSON）。
