...
助手的"意图识别"不写死在代码里，而是一个**可维护的技能库**——在
价格维护面板的 **技能库** 标签页里维护：

![技能库](img/ui-skills.png)

- 三类技能：**动作**（绑定工具，如报价/DFM）、**分析**（工具链）、**知识**（FAQ 直接答）。
- 默认开箱即用 15 条，含 7 条工艺知识 FAQ（公差/深孔/薄壁/选材/表面/单位/批量）。
- **人工干预**：改触发词、改话术、临时启停、新增自定义技能——全程不改代码，
  覆盖叠加在默认库上、可回退、有审计。内置技能的"绑定工具"不可改（防静默改坏），
  其余可编辑。
- provider 无关：Mock 用它规则匹配；上线真 LLM 作为系统提示/few-shot 注入。
...
| 标签页 | 维护内容 |
|--------|----------|
| 材料 | 单价、密度、机加工性、毛坯系数、废料抵扣率、抗拉强度、备料周期;**可新增整种自定义材料**(下图) |
| 机床 | 时租、基础 MRR、最大轴数;**可新增整台自定义机床** |
| 表面处理 | 起步/每dm²/保底、外协周期;**可新增整种自定义工艺**(电镀/PVD 等, 需指定适用材料) |
...
#### 新增自定义条目 Add materials / finishes / machines

材料、表面处理、机床**三个目录均可手动新增**,无需改代码或重启——同一套
"默认兜底 + 人工自治 + 可回退审计"三层模式:

- **材料**(下图):代号/名称/类别/密度/单价/机加工性(必填)+ 强度/废料抵扣/适用表面(选填)
- **表面处理**:如电镀/PVD——必须勾选**适用材料**(否则新工艺无处可用被拒);
  删除时自动从材料选项中收回;外协天数计入交期
- **机床**:代号/名称/时租/基础 MRR/轴数(限 3/4/5);新机床立即可在报价表单指定

新增的条目**立即进下拉、可报价、可逐字段改价、可删除**,与内置一视同仁;
内置条目不可被覆盖或删除;全程审计。

![新增材料](img/ui-custom-material.png)

填代号/名称/类别(金属/塑料)/密度/单价/机加工性(必填)+ 抗拉强度/废料抵扣率/
适用表面处理(选填)→ 保存后**立即进材料下拉、可报价、可逐字段改价、可删除**,
与内置材料一视同仁。内置材料不可被覆盖或删除;全程审计。
